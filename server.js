...
const lxmusicApi = require('./lxmusic-api');
const { logLxMusic } = require('./lxmusic-debug-log');
const {
...
const server = http.createServer(async (req, res) => {
  ...
  if (pn === '/api/platform/capabilities') {
    ...
    sendJSON(res, {
      ...
      lxmusic: {
        search: false, url: true, playlists: false,
        likeRead: false, likeWrite: false,
        albumRead: false, albumCollect: false,
        commentsRead: false, commentsWrite: false,
        listenReport: false, loginRequired: false,
      },
    });
    return;
  }
  ...
  if (pn === '/api/lxmusic/log') {
    try {
      const body = await readRequestBody(req);
      logLxMusic(body.category || 'CLIENT', body.message || '', body.data || null);
  ...
  if (pn === '/api/lxmusic/status') {
    try {
      var statusResult = lxmusicApi.getLxMusicStatus();
      logLxMusic('STATUS', '状态查询', statusResult);
      sendJSON(res, statusResult);
    } catch (err) {
      logLxMusic('ERROR', '状态查询失败', { error: err.message });
      console.error('[LxMusicStatus]', err);
      sendJSON(res, { error: err.message }, 500);
    }
    return;
  }

  if (pn === '/api/lxmusic/config') {
    try {
      const body = await readRequestBody(req);
      const saved = lxmusicApi.saveLxMusicConfig(body);
      const masked = Object.assign({}, saved, {
        backends: (saved.backends || []).map(function (b) { return Object.assign({}, b, { key: '***' }); }),
      });
      sendJSON(res, masked);
    } catch (err) {
      console.error('[LxMusicConfig]', err);
      sendJSON(res, { error: err.message }, err.code === 'INVALID_LX_CONFIG' ? 400 : 500);
    }
    return;
  }

  if (pn === '/api/lxmusic/song/url') {
    try {
      const source = url.searchParams.get('source') || 'wy';
      const id = url.searchParams.get('id') || '';
      const quality = url.searchParams.get('quality') || 'hires';
      sendJSON(res, await lxmusicApi.resolveLxMusicUrl({ source: source, songId: id, quality: quality }));
    } catch (err) {
      console.error('[LxMusicSongUrl]', err);
      sendJSON(res, { provider: 'lxmusic', playable: false, error: err.message }, 500);
    }
    return;
  }

  if (pn === '/api/lxmusic/resolve') {
    try {
      ...
      const mixSongId = url.searchParams.get('mixSongId') || '';
      ...
      var lxSource;
      if (provider === 'netease') lxSource = 'wy';
      else if (provider === 'qq') lxSource = 'tx';
      else if (provider === 'kugou') lxSource = 'kg';
      else lxSource = 'wy';

      var songId = id || mid || songmid || hash || mixSongId || '';
      logLxMusic('RESOLVE', '映射结果', { lxSource, songId });
      var result;

      if (songId) {
        // Direct lookup by source + id, use selected backend if specified
        var resolveOpts = {
          source: lxSource,
          songId: songId,
          quality: quality,
      ...
      // If no direct result or no id, try cross-search via qishui/spotify providers
      if (!result || result.playable !== true) {
        logLxMusic('RESOLVE', '直接解析失败，尝试跨平台搜索', { provider });
        var query = [name, artist].filter(Boolean).join(' ').trim();
        if (query && (provider === 'qishui' || provider === 'spotify')) {
          try {
            var searchResults = provider === 'qishui'
              ? await handleSearch(query, 12)
              : await handleQQSearch(query, 12);

            if (Array.isArray(searchResults)) {
              for (var si = 0; si < searchResults.length; si++) {
                var sr = searchResults[si];
                var srName = (sr && sr.name || '').toLowerCase();
                var songName = name.toLowerCase();
                var srDuration = Number(sr && sr.duration) || 0;
                if (srName === songName && (!duration || !srDuration || Math.abs(srDuration - duration) < 5000)) {
                  var srId = sr && (sr.id || sr.songmid || sr.mid || '');
                  if (srId) {
            ...
            logLxMusic('ERROR', '跨平台搜索失败', { error: searchErr && searchErr.message });
          }
        }
      }
      ...
      logLxMusic('ERROR', '解析异常', { error: err.message, stack: err.stack && err.stack.substring(0, 200) });
      sendJSON(res, { provider: 'lxmusic', playable: false, error: err.message }, 500);
    }
    return;
  }
...
server.listen(PORT, HOST, () => {
  ...
  // Initial health check for lxmusic backends
  try {
    var lxConfig = lxmusicApi.getLxMusicConfig();
    var backends = lxConfig.backends || [];
    backends.forEach(function (b) {
      var controller = new AbortController();
      var timer = setTimeout(function () {
        controller.abort();
        lxmusicApi.updateBackendStatus(b.id, false, 'startup_timeout');
        console.log('[LxMusic] ' + b.name + ' → 离线 (超时)');
      }, 5000);
      var url = b.baseUrl;
      fetch(url, { method: 'HEAD', signal: controller.signal })
        .then(function () {
          clearTimeout(timer);
          lxmusicApi.updateBackendStatus(b.id, true);
          console.log('[LxMusic] ' + b.name + ' → 在线');
        })
        .catch(function (err) {
          clearTimeout(timer);
          lxmusicApi.updateBackendStatus(b.id, false, err.message);
          console.log('[LxMusic] ' + b.name + ' → 离线 (' + (err.message || '未知错误') + ')');
        });
    });
  } catch (e) { /* ignore */ }
});
