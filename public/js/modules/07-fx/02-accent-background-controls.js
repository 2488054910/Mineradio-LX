...
function applyCustomBackground() {
  ...
  // 综合背景透明度：背景透明度与窗口背景透明共同决定，越低越透出桌面
  var effectiveOpacity = Math.min(windowOpacity, opacity);
  ...
  var override = albumMode || !!media || customColor || glassActive;
  var root = document.documentElement;
  ...
  root.style.setProperty('--custom-bg-album-opacity', effectiveOpacity.toFixed(3));
  ...
  var windowTransparent = effectiveOpacity < 0.999;
  document.documentElement.classList.toggle('custom-window-transparent', windowTransparent);
  document.body.classList.toggle('custom-window-transparent', windowTransparent);
  ...
  if (layer) {
    ...
    layer.style.setProperty('--custom-bg-image-opacity', image ? effectiveOpacity.toFixed(3) : '0');
    layer.style.setProperty('--custom-bg-video-opacity', hasVideo ? effectiveOpacity.toFixed(3) : '0');
    layer.style.setProperty('--custom-bg-base-opacity', effectiveOpacity.toFixed(3));
