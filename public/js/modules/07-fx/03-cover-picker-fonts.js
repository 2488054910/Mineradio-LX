...
function preloadBuiltinLyricFont(key) {
  if (!/^wenkai/.test(String(key || ''))) return Promise.resolve(false);
  var weight = lyricFontWeightValue();
  var stack = lyricFontStackForKey(key);
  var family = stack && stack.split(',')[0];
  if (!family || typeof document === 'undefined' || !document.fonts || typeof document.fonts.load !== 'function') return Promise.resolve(false);
  return document.fonts.load(weight + ' 128px ' + family).then(function () {
    clearLyricTextMeasureCache();
    scheduleLyricTextMeasureWarmup(0);
    if (normalizeLyricFontKey(fx && fx.lyricFont) === key) {
...
function setLyricFont(key) {
  ...
  pushDesktopLyricsState(true);
  showToast('歌词字体已切换');
  preloadBuiltinLyricFont(fx.lyricFont);
}
