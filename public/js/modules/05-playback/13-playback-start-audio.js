...
async function resolveAlbumGaplessPlaybackData(song) {
  ...
  if (playbackProvider === 'lxmusic') {
    return apiJson('/api/lxmusic/song/url?source=' + encodeURIComponent(song.lxSource || 'wy') + '&id=' + encodeURIComponent(song.lxSongId || song.id || '') + qualityParam, { timeoutMs: 10000 });
  }
...
async function playQueueAt(idx, opts) {
  ...
  if (typeof currentPlaybackFreeSource !== 'undefined' && !(opts && opts.freeSourceResolved)) currentPlaybackFreeSource = null;
    ...
    try {
      ...
      if (albumGaplessHandoff) {
        ...
        data = await apiJson('/api/spotify/song/url?id=' + encodeURIComponent(song.id || song.providerSongId || song.spotifyId || '') +
          ...
          '&uri=' + encodeURIComponent(song.spotifyUri || song.uri || '') +
          qualityParam, { timeoutMs: 9000 });
      } else if (playbackProvider === 'lxmusic') {
        data = await apiJson('/api/lxmusic/song/url?source=' + encodeURIComponent(song.lxSource || 'wy') + '&id=' + encodeURIComponent(song.lxSongId || song.id || '') + qualityParam, { timeoutMs: 10000 });
      } else {
      ...
      if (data.trial) {
        // Try free source fallback for trial songs before showing trial banner
        if (typeof tryLxMusicFreeFallback === 'function' && !opts.freeSourceResolved && !opts._lxFreeAttempted) {
          try {
            var _logTrial = { name: song && (song.name || song.title), provider: playbackProvider, trial: data.trial, restriction: data.restriction, loggedIn: data.loggedIn, vipLevel: data.vipLevel };
            fetch('/api/lxmusic/log', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ category: 'TRIAL', message: '检测到试听限制，尝试免费音源', data: _logTrial }) }).catch(function () {});
          } catch (_) {}
          var trialRestriction = Object.assign({}, data, { restriction: { category: 'trial_only' } });
          var lxFreeResult = await tryLxMusicFreeFallback(song, trialRestriction, idx, token, Object.assign({}, opts, { _lxFreeAttempted: true }));
          if (lxFreeResult === true) {
            // Free source played successfully, stop here
            return true;
          }
        }
        // Free source failed or not available, show trial banner
        var txt;
      ...
      // Update free-source dropdown display
      if (typeof currentPlaybackFreeSource !== 'undefined') {
        if (typeof updateFreeSourceButton === 'function') {
