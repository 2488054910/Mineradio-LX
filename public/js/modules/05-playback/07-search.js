...
function songProviderKey(song) {
  if (song && (song.provider === 'lxmusic' || song.source === 'lxmusic')) return 'lxmusic';
...
function songSourceTagHtml(song, opts) {
  ...
  var key = /^(netease|qq|kugou|qishui|spotify|lxmusic)$/.test(String(rawKey || '')) ? String(rawKey) : songProviderKey(song);
  var label = key === 'qq' ? 'QQ' : (key === 'kugou' ? 'KG' : (key === 'qishui' ? 'QS' : (key === 'spotify' ? 'SP' : (key === 'lxmusic' ? '免费' : 'NE'))));
  if (opts.switcher) {
