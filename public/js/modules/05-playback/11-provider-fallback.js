...
function playbackProviderLabel(song) {
  ...
  if (provider === 'lxmusic') return '免费音源';
  return '网易云';
}
...
function ensureRestrictionCategory(data, category) {
  // 确保 data.restriction.category 存在且在 tryLxMusicFreeFallback 的 Guard 4 触发列表中
  // lxFreeTriggerCategories = ['vip_required','paid_required','trial_only','copyright_unavailable','url_unavailable']
  // login_required 不在列表中，需要映射为 url_unavailable
  data = data || {};
  var currentCat = data.restriction && data.restriction.category;
  if (currentCat && currentCat !== 'login_required') return data; // 已在触发列表中
  var mappedCategory = (category === 'login_required' || currentCat === 'login_required') ? 'url_unavailable' : (category || currentCat || 'url_unavailable');
  return Object.assign({}, data, { restriction: Object.assign({}, data.restriction || {}, { category: mappedCategory }) });
}
...
async function skipFailedQueueItem(idx, token, message, opts) {
  ...
  var notice = playbackRestrictionNotice(song, opts.data || {});
  var detailedMessage = message || notice.body || '未找到同名同歌手的另一个平台版本，正在播放下一首。';
  if (!opts.silent) showSourceFallbackNotice('已跳过受限歌曲', detailedMessage);
...
async function tryAutoPlaybackFallback(song, data, idx, token, opts) {
  ...
  console.log('[SourceFallback] tryAutoPlaybackFallback:', song && (song.name || song.title), 'category:', category, 'alternateProviders:', alternateProviders.length, 'data.restriction:', data && data.restriction);
  // 优先尝试免费音源：只要原始平台不可用（login_required等），先尝试免费音源
  if (category === 'login_required' || category === 'vip_required' || category === 'trial_only' || category === 'copyright_unavailable' || category === 'url_unavailable') {
    console.log('[SourceFallback] ' + category + ', trying free source first');
    if (typeof tryLxMusicFreeFallback === 'function') {
      var lxFreeData1 = ensureRestrictionCategory(data, category);
      var lxMusicFallbackEarly = await tryLxMusicFreeFallback(song, lxFreeData1, idx, token, opts);
      console.log('[SourceFallback] free source result:', lxMusicFallbackEarly);
      if (lxMusicFallbackEarly === true) return true;
    }
    // 免费音源失败，如果有其他已登录平台则继续换源
    if (alternateProviders.length) {
      console.log('[SourceFallback] free source failed, trying alternate providers');
    } else {
      return null;
    }
  }
  ...
  if (typeof tryLxMusicFreeFallback === 'function' && sourceFallbackRecoveryCanContinue(recovery)) {
    var lxFreeData3 = ensureRestrictionCategory(data, category);
    var lxMusicFallback = await tryLxMusicFreeFallback(song, lxFreeData3, idx, token, opts);
    if (lxMusicFallback === true) {
      if (typeof completeSourceFallbackRecovery === 'function') completeSourceFallbackRecovery(recovery);
      return true;
    }
  }
