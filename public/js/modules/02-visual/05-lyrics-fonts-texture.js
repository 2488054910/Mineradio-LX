function builtinLyricFontKeyPattern() {
  return /^(sans|hei|song|bold-song|stone-song|kai-song|serif-en|gothic|editorial|humanist|round|mono|display|wenkai|wenkai-light|wenkai-medium|wenkai-mono|wenkai-mono-light|wenkai-mono-medium)$/;
}
...
function preloadBuiltinLyricFontIfSaved() {
  if (typeof fx === 'undefined' || !fx || !/^wenkai/.test(String(fx.lyricFont || ''))) return;
  if (typeof document === 'undefined' || !document.fonts || typeof document.fonts.load !== 'function') return;
  var weight = lyricFontWeightValue();
  var stack = lyricFontStackForKey(fx.lyricFont);
  var family = stack && stack.split(',')[0];
  if (!family) return;
  document.fonts.load(weight + ' 128px ' + family).then(function () {
    clearLyricTextMeasureCache();
    scheduleLyricTextMeasureWarmup(0);
  }).catch(function () { });
}
if (typeof document !== 'undefined') {
  setTimeout(preloadBuiltinLyricFontIfSaved, 400);
}
...
function lyricFontStackForKey(key) {
  ...
  if (key === 'wenkai') return '"LXGW WenKai","Kaiti SC","STKaiti","KaiTi","Noto Serif SC",serif';
  if (key === 'wenkai-light') return '"LXGW WenKai Light","LXGW WenKai","Kaiti SC","STKaiti","KaiTi",serif';
  if (key === 'wenkai-medium') return '"LXGW WenKai Medium","LXGW WenKai","Kaiti SC","STKaiti","KaiTi",serif';
  if (key === 'wenkai-mono') return '"LXGW WenKai Mono","LXGW WenKai","JetBrains Mono",Consolas,monospace';
  if (key === 'wenkai-mono-light') return '"LXGW WenKai Mono Light","LXGW WenKai Mono","LXGW WenKai","JetBrains Mono",Consolas,monospace';
  if (key === 'wenkai-mono-medium') return '"LXGW WenKai Mono Medium","LXGW WenKai Mono","LXGW WenKai","JetBrains Mono",Consolas,monospace';
