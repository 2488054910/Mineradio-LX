...
function canShowShelfHoverCueAt(e) {
  if (!e) return false;
  if (shelfPlaybackSwitchGuardActive()) return false;
  // 在自动隐藏模式下，不检查 guide 标志，允许 hover cue 触发
  if (fx && fx.shelfPresence !== 'auto' && !shelfHoverCue.guide) return false;
...
function updateShelfHoverCueFromPointer(e) {
  ...
  // 检查鼠标是否在卡片上
  var overCard = false;
  if (shelfManager && shelfManager.getMode && shelfManager.getMode() === 'side' && shelfVisibility > 0.3) {
    var hit = pointerCardHit(raycasterFromPointerEvent(e), e, 18);
    overCard = !!hit;
  }
  if (inZone && !shelfHoverCue.zoneActive) {
    shelfHoverCue.zoneActive = true;
    shelfHoverCue.enteredAt = performance.now();
  } else if (!inZone && !overCard) {
    shelfHoverCue.zoneActive = false;
    shelfHoverCue.enteredAt = 0;
  }
  active = inZone || overCard;
...
function tickShelfHoverCue(dt) {
  ...
  if (!shelfHoverCue.guide && shelfHoverCue.zoneActive) {
    ...
    if (canShowShelfHoverCueAt(heldPointer)) {
      ...
      var overCard = false;
      if (shelfManager && shelfManager.getMode && shelfManager.getMode() === 'side' && shelfVisibility > 0.3) {
        var hit = pointerCardHit(raycasterFromPointerEvent(heldPointer), heldPointer, 18);
        overCard = !!hit;
      }
      // 只有当鼠标不在触发区域且不在卡片上时，才隐藏
      if (!overCard) {
        shelfHoverCue.zoneActive = false;
        shelfHoverCue.enteredAt = 0;
        shelfHoverCue.target = 0;
